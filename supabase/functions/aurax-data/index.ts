...
const FIREBASE_WEB_API_KEY = Deno.env.get("AURAX_FIREBASE_WEB_API_KEY") || "AIzaSyC4thDZHipZQSeZGyKXzeh-m20EC_-rcKg";
...
async function loginWithUsername(body: any) {
  const username = String(body.username || "").trim();
  const password = String(body.password || "");
  if (!username || !password) {
    throw Object.assign(new Error("Kullanıcı adı ve şifre gereklidir."), { status: 400, code: "AUTH_REQUIRED" });
  }
  if (!/^[A-Za-z0-9_.-]{3,32}$/.test(username)) {
    throw Object.assign(new Error("Kullanıcı adı veya şifre hatalı."), { status: 401, code: "AUTH_INVALID" });
  }

  // Normal kullanıcıların UID/e-posta eşleşmesi aurax_identities'den gelir.
  // Eski/admin hesaplarda identity satırı eksik kalmışsa aynı kullanıcı kaydını
  // ve aktif admin kaydını kontrollü biçimde fallback olarak kullanıyoruz.
  let email: string | null = null;
  let expectedUid: string | null = null;
  let resolvedUsername = username;

  const { data: identity, error: identityError } = await adminDb
    .from("aurax_identities")
    .select("firebase_uid,email,username")
    .eq("username", username)
    .maybeSingle();
  if (identityError) {
    throw Object.assign(new Error("Giriş sunucusunun kimlik tablosu okunamadı."), { status: 500, code: "AUTH_BACKEND" });
  }

  if (identity?.email) {
    email = identity.email;
    expectedUid = identity.firebase_uid || null;
    resolvedUsername = identity.username || username;
  } else {
    const { data: adminRow, error: adminError } = await adminDb
      .from("aurax_admins")
      .select("firebase_uid,username,active")
      .eq("username", username)
      .eq("active", true)
      .maybeSingle();
    if (adminError) {
      throw Object.assign(new Error("Giriş sunucusunun yönetici tablosu okunamadı."), { status: 500, code: "AUTH_BACKEND" });
    }
    ...
    const { data: userRow, error: userError } = await adminDb
      ...
      .maybeSingle();
    if (userError) {
      throw Object.assign(new Error("Aura X kullanıcı kaydı okunamadı."), { status: 500, code: "AUTH_BACKEND" });
    }

    const userData = userRow?.data || {};
    email = String(userData.googleEmail || userData.email || "").trim() || null;
    expectedUid = adminRow?.firebase_uid || userData.firebaseUid || userData.googleUid || null;
    if (adminRow?.username) resolvedUsername = adminRow.username;
  }
  ...
  try {
    // Gerçek e-posta adresi, şifre doğrulanmadan istemciye açılmaz.
    // Firebase Auth'un resmi Identity Toolkit uç noktasıyla kimlik bilgilerini
    // sunucu tarafında doğruluyoruz.
    ...
    // Identity/admin UID varsa, aynı kullanıcı hesabının doğrulandığından emin ol.
    if (expectedUid && payload.localId !== expectedUid) {
      throw Object.assign(new Error("Kullanıcı hesabı kimlik eşleşmesi başarısız."), { status: 401, code: "AUTH_INVALID" });
    }

    return { ok: true, email, username: resolvedUsername, firebaseUid: payload.localId };
  } catch (error) {
    if ((error as any)?.code === "AUTH_INVALID") throw error;
    console.error("Firebase username login error:", error);
    throw Object.assign(new Error("Giriş sunucusuna ulaşılamadı."), { status: 502, code: "AUTH_PROVIDER_UNREACHABLE" });
  }
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Kullanıcı adı + şifre doğrulaması login öncesi kullanılabilir; diğer tüm
    // işlemler aşağıdaki Firebase ID token doğrulamasından geçmeye devam eder.
    if (op === "login_username") return json(await loginWithUsername(body));
