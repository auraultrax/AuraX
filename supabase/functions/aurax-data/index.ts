...
async function loginWithUsername(body: any) {
  ...
  let resolvedUsername = identity?.username || username;

  // Identity tablosunda kayıt yoksa veya eski hesapta identity kaydı eksikse,
  // Firebase Auth e-postasını mevcut kullanıcı belgesinden yalnızca sunucu
  // tarafında çözümleriz. Böylece admin dahil eski hesaplar da çalışır.
  if (!email) {
    const { data: userRow, error: userError } = await adminDb
      ...
      .eq("doc_id", username)
      .maybeSingle();
    if (userError) throw Object.assign(new Error("Kullanıcı kaydı çözümlenemedi."), { status: 500, code: "USER_LOOKUP_FAILED" });
    email = userRow?.data?.googleEmail || userRow?.data?.email || null;
  }

  // A valid username must resolve to an email before Firebase Auth is queried.
  if (!email) {
    throw Object.assign(new Error("Kullanıcı adı veya şifre hatalı."), { status: 401, code: "AUTH_INVALID" });
  }
  ...
  // burada doğruluyoruz; tarayıcı yalnızca başarılı doğrulamadan sonra email alır.
  ...
  return { ok: true, email, username: resolvedUsername };
}
